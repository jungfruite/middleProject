...
@ServerEndpoint(value = "/mbtiPrivateChat.do", configurator = HttpSessionConfigurator.class)
public class MbtiPrivateChat {
	...
	public static String checkRoom(String mbtiCode, String chatMbtiSelect) {
		Set<String> keySet = sessionUsersMap.keySet();
		for (String room : keySet) {
			if(room.startsWith(mbtiCode) && room.endsWith(chatMbtiSelect) && (sessionUsersMap.get(room).size() < 2) && room.length() > 4) {
				return room;
			}
		}
		return "0";
	...
	public void handleOpen(Session userSession){
		...
		String mbtiCode = mv.getMbticode();
		String memberId = mv.getMemberId();
		String chatMbtiSelect = (String) session.getAttribute("chatMbtiSelect");
		String roomName = (String) session.getAttribute("privateRoomName");
		System.out.println(roomName);
		if(!sessionUsersMap.containsKey(roomName)) {
			sessionUsersMap.put(roomName, new ArrayList<ChatMemberVO>() );
		}
		ChatMemberVO chatVo = new ChatMemberVO(mv.getMemberNickname(), userSession);
		sessionUsersMap.get(roomName).add(chatVo);
		System.out.println(mv.getMemberId()+" / "+mv.getMemberNickname()+" : "+ roomName+ " 채팅방 접속\n");
	}
	...
	public void handleMessage(String message, Session userSession) throws IOException{
		...
		String room =  (String) session.getAttribute("privateRoomName");
		...
		if(username == null || "connect".equals(chatMessageVo.getCommand()) ){
			for(ChatMemberVO chatVo : sessionUsersMap.get(room)){
				if(userSession.equals(chatVo.getSession())){
					...
					if(sessionUsersMap.get(room).size() == 2) {
					userSession.getBasicRemote().sendText(buildJsonData("System", chatMbtiSelect+" 유형과 매칭 성공! "+ mv.getMemberNickname() + "님이 입장했습니다.", userSession));
					
					Iterator<ChatMemberVO> iterator = sessionUsersMap.get(chatMessageVo.getRoom()).iterator();
					while(iterator.hasNext()){
						ChatMemberVO chVo = iterator.next();
						if(!chVo.getSession().equals(chatVo.getSession())){
							chVo.getSession().getBasicRemote().sendText(buildJsonData("System",mv.getMbticode()+" 유형과 매칭 성공! "+ mv.getMemberNickname() + "님이 입장했습니다.", userSession));
						}
					}
					}
